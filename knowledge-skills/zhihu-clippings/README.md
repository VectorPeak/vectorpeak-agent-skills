...
1. **输入识别**：接受作者名、知乎 people URL、文章标题、文章 URL、article_id、question/answer ID 或截图转写标题列表
...
3. **正文补全**：使用 TikHub 的知乎接口获取完整正文
   ...
   - 问答内容：按脚本支持的 question/answer 模式处理
...
```powershell
python .\scripts\clip_zhihu_official.py "何宇峰" --author "何宇峰" --count 10 --output-dir "<your-obsidian-vault>\Clippings\Zhihu"
```
...
`{summary}` 是根据文章标题推断出的短主题，例如 `大模型Agent`、`算法面试` 或 `大模型架构与注意力机制`
