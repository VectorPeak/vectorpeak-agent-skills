...
<p align="center">
  把每天学习时产生的短期目标、疑问、代码记录、概念和踩坑经验，轻量追加到 raw 层的 10 天周期 DailyNotes Markdown 文件中
</p>
...
```text
零散学习输入  ->  Goal / Question / Code / Concept / Pitfall 分类  ->  10 天一个 Markdown  ->  后续整理到 Wiki
```
...
学习时最容易丢的不是完整文章，而是那些随手冒出来的小目标、疑问、代码片段、概念碎片和踩坑经验。

Daily Notes VP 解决的是“快速记录，不打断学习流”的问题。当用户说“记一下这个问题”“今天的目标是”“这个代码点先放到 daily notes”“这个概念先记一下”时，Agent 不需要生成一篇完整 Wiki，也不需要追问复杂分类，而是把内容轻量追加到 raw 原始语料层。

核心策略是 **raw-first, lightweight, reviewable**：

- 按 10 天一个文件聚合，避免单日文件太碎
- 每天内部固定按 `Goal-目标 -> Question-疑问 -> Code-代码 -> Concept-概念 -> Pitfall-踩坑` 排列
- 不记录具体时间点，标题直接使用问题、目标、代码点、概念或踩坑名称
...
这个代码片段先记一下
这个概念先记一下
把这个疑问放到 E:\LLM_wiki\LLM_wiki\01.raw\02.DailyNotes
用 daily-notes-vp 记录
```
...
- `daily-notes-vp` 记录个人日常学习原始材料，默认位置是 `E:\LLM_wiki\LLM_wiki\01.raw\02.DailyNotes`
